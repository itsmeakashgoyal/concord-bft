...
bool SKVBCTestsBuilder::lookForConflicts(uint64_t readVersion, const vector<vector<uint8_t>>& readKeysArray) {
...
void SKVBCTestsBuilder::addNewBlock(const vector<pair<vector<uint8_t>, vector<uint8_t>>>& writesKVArray) {
  ++lastBlockId_;
...
void SKVBCTestsBuilder::createAndInsertRandomConditionalWrite() {
  // Create request
