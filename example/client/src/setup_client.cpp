...
void SetupClient::ParseClientArgs(int argc, char** argv) {
  ...
  try {
    ...
    std::unique_ptr<bft::communication::ICommunication> comm(setupCommunicationParams());
...
ClientConfig SetupClient::setupClientConfig() {
  ClientConfig bftClientConf;
  bftClientConf.f_val = clientParams_.numOfFaulty;
  bftClientConf.c_val = clientParams_.numOfSlow;
  bftClientConf.id = clientParams_.clientId;
...
ICommunication* SetupClient::setupCommunicationParams() {
  TestCommConfig testCommConfig(logger_);
  uint16_t numOfReplicas = clientParams_.get_numOfReplicas();
#ifdef USE_COMM_PLAIN_TCP
  PlainTcpConfig conf = testCommConfig.GetTCPConfig(
      false, clientParams_.clientId, clientParams_.numOfClients, numOfReplicas, clientParams_.configFileName);
