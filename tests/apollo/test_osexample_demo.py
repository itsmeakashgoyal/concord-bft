...
class OSExampleDemoTest(ApolloTest):
    ...
    async def test_osexample_demo(self, bft_network):
        self._run(bft_network)


    def _run(self, bft_network):
        """
        ...
        with log.start_action(action_type="run osexample demo script"):
            ...
            if os.environ.get('KEEP_APOLLO_LOGS', "").lower() in ["true", "on"]:
                ...
                demo_script = os.path.join(bft_network.builddir, "examples", "scripts", "test_example.sh")
                ...
                demo_script_pid = subprocess.Popen(
                    demo_script_cmd,
                    stdout=stdout_file,
                    stderr=stderr_file,
                    close_fds=True)
                try:
                    exit_code = demo_script_pid.wait()
                    assert exit_code == 0
